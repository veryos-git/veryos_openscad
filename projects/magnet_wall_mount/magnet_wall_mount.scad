...
/* [Cover thickness] */
// Material between the magnet and the exposed (outer) face.
// Thinner cover = stronger magnetic pull at the surface.
thick_top = 0.6;

/* [Bracket] */
tolerance          = 0.2;
thick_side         = 1.2;   // material around the magnet pocket
width_screw_holder = 12;    // lateral space allocated for each screw column
chamfer            = 0.5;   // 45° chamfer on vertical edges and counterbore openings
floor_min          = 1;     // minimum solid material between counterbore and wall face
wall_min           = 1;     // minimum solid material around counterbore (lateral)
...
// Minimum lateral extent needed to keep wall_min material around the
// counterbore at the top face. Top edges aren't chamfered (edges = "Z"),
// so only the counterbore's own chamfer cone widens the opening — by
// `chamfer` per side.
counterbore_min = screw_head_diameter + 2 * chamfer + 2 * wall_min;

flange_w = max(width_screw_holder, counterbore_min);

bL = pL + 2 * thick_side + 2 * flange_w;
bW = max(pW + 2 * thick_side, counterbore_min);
// Block tall enough for the magnet pocket and for the counterbore + floor
bH = max(pH + thick_top, counterbore_depth + floor_min);

screw_x = pL/2 + thick_side + flange_w/2;

eps = 0.01;

module bracket() {
    difference() {
        cuboid([bL, bW, bH], chamfer = chamfer, edges = "Z", anchor = BOTTOM);
        if (is_round)
            translate([0, 0, -eps])
